...
- **CRE workflows:** Run offchain (CRE network). They verify EIP-712 signatures, call compliance/KYC APIs privately, store bids, compute clearing price, and call back onchain to SilentBid (`BlindPoolCCA.forwardBidToCCA` / `forwardBidsToCCA`).
...
## Onchain: SilentBid (BlindPoolCCA)
...
2. Frontend builds an EIP-712 message (e.g. `BlindPoolBid`: sender, auctionId, maxPrice, amount, flags, timestamp) and asks the user to sign.
...
4. **Option B (direct commit):** Frontend computes `bidCommitment = keccak256(abi.encodePacked(...))` and calls `BlindPoolCCA.submitBlindBid(bidCommitment)` with `msg.value = amount`. The same signed message can be sent to CRE for compliance and storage so the workflow has the plaintext for finalization.
...
- SilentBid contract and scripts: **BlindPool-scripts** repo; see README for deploy and forward flows.
